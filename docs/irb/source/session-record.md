...
- ☐ Institutionally reviewed OpenVoice V2 release/commit and model weights installed on the UCF-managed encrypted voice workstation.
- ☐ Workstation network access disabled during participant voice processing; automatic cloud backup/sync disabled for temporary study folders.
- ☐ Workstation access limited to approved staff; fixed prompt manifest loaded.
- ☐ No cloud voice credential, upload path, or provider API call exists in the workstation workflow or headset build/logs.
...
- ☐ Baseline 16-item SSQ completed before headset use.
- ☐ $15 gift card is owed once screening began, regardless of disposition.

## Voice Procedure

- Recording script/version: ____________________  Take: ____________________  Duration: ____________________
- ☐ No name or direct identifier in recording/file/local embedding alias.
- OpenVoice release/commit, weights, base TTS, and settings: ____________________
- Local embedding alias: ____________________  Stored only in restricted deletion log: ☐ Yes
- Offline create result: ☐ Success ☐ Failed ☐ Not attempted
...
- Voice-generation issue; maximum two takes/one regeneration followed: ☐ Yes ☐ No ☐ N/A
- If unsuccessful, session stopped before randomized blocks and all voice artifacts deleted: ☐ Yes ☐ No ☐ N/A
...
- Eye calibration result: ☐ Pass ☐ Fail  Retries: ______ (maximum 2)  Quality/validity output: ____________________
- Nine-direction validation: targets detected: ____/9  Median error: ____°  90th percentile: ____°  Gate met (≥8/9, ≤3°, ≤6°): ☐ Yes ☐ No
...
- ☐ Nine fixed manipulation/experience checks completed after each block; two mechanism checks added after gaze-contingent blocks only.
- ☐ Minimum two-minute seated break provided after blocks 1–3 or longer on request.
- ☐ End-of-session instruments completed or skips recorded without pressure.
- ☐ Post-exposure 16-item SSQ completed after headset removal.
- ☐ Each standard 16-trial condition manifest places the target exactly twice on each of the eight planes.
- ☐ Any block/session ending before 16 trials has a prespecified reason recorded; trial count was not changed because of participant performance or condition results.
- ☐ Search-onset, seven scheduled hint opportunities (4, 10, 16, 22, 28, 34, and 40 seconds), policy evidence/state, playback outcome, selection, 45-second timeout, and trial-end events logged.
...
- Local source recording deleted within 24 hours: ☐ Yes ☐ No  Date/time/result: ____________________
- Local speaker embedding/model artifact deleted within 24 hours: ☐ Yes ☐ No  Date/time/result: ____________________
- Workstation generated clips/session cache deleted within 24 hours: ☐ Yes ☐ No  Date/time/result: ____________________
- Headset generic/self-similar session clips/cache deleted within 24 hours: ☐ Yes ☐ No  Date/time/result: ____________________
- Identity/linkage record updated under approved plan: ☐ Yes ☐ No
- Any deletion failure: access restricted ☐ N/A ☐ Yes  PI notified within 24 hours ☐ N/A ☐ Yes
...
Compensation: $15 gift card ☐ Issued ☐ Owed  Payment record location: ____________________
...
- ☐ Trial tracking availability below 70% valid samples
...
Complete-dataset quality check: at least 12 trials with at least 70% valid gaze
samples in each cell ☐ Yes ☐ No. Observable assignment-policy outcomes are retained
where possible; performance outliers alone are not exclusion reasons.
